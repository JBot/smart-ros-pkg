...
{
    ros::init(argc, argv, "arduIMU_publisher");
    ros::NodeHandle n;
    ...
    priv_nh.param("baud_rate", baud_rate, 38400);
...
//                ROS_ERROR("step3");
            ...
            if (init_level == 4) {
                ...
                // roll
                boost::asio::read(serial_, boost::asio::buffer(&roll_, 2));
                roll = (((double)( (int16_t)((int16_t)roll_[1] << 8) + (int16_t) roll_[0]) ) / 100.0) * DEG2RAD;
                ...
                pitch = (((double)( (int16_t)((int16_t)pitch_[1] << 8) + (int16_t) pitch_[0]) ) / 100.0) * DEG2RAD;

                ROS_ERROR("pitch %f", pitch);

                boost::asio::read(serial_, boost::asio::buffer(&yaw_, 2));
                yaw = (((double)( (int16_t)((int16_t)yaw_[1] << 8) + (int16_t) yaw_[0]) ) / 100.0) * DEG2RAD;
